...
#    Updated: 2025/10/21 09:10:41 by joaolive         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
# default rule: compile ALL modules
$(NAME): $(ALL_OBJS)
	@$(AR) $@ $^
	@echo "🎉 $(NAME) compiled successfully with ALL modules 🚀"
...
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.c
	@mkdir -p $(dir $@)
	@printf "  $(CC_BLUE)[CC]$(RESET) Compiling $(CC_YELLOW)%s$(RESET) -> $(CC_PINK)%s$(RESET)\n" $< $@
